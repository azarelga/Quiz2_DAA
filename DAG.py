class TaskSchedulerDAG:
    ...
    def add_dependency(self, prerequisite_task, dependent_task):
        ...
        for neighbor in self.graph[dependent_task]:
            if (neighbor == prerequisite_task) or (prerequisite_task in self.graph[neighbor]):
                raise ValueError(f"Cannot add dependency: {dependent_task} already depends on {prerequisite_task}, which creates a cycle.")
